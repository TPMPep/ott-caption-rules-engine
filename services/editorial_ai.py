        ...
        response = client.responses.create(
            ...
            input=[
                {"role": "system", "content": (
                    ...
                    "Return at most two lines, each 32 characters or fewer. "
                    ...
                    "Prefer keeping full phrases together over balancing line lengths. "
                    "Reject awkward fragments such as 'And I', 'It's', 'Who made', 'What was your', 'just go home', or 'little bit about his' if a better break is possible. "
                    "If punctuation is ambiguous and the thought clearly continues, prefer a comma rather than a period. "
...
def _has_bad_period_lowercase_sequence(edited: str) -> bool:
    return bool(re.search(r"(?<![A-Z])[A-Za-z]\.(?:\s+)([a-z])", edited))
...
def _has_weak_one_word_line(lines: List[str]) -> bool:
    ...
    for line in lines:
        ...
        if len(words) == 1 and words[0].lower() in weak:
            return True
    if len(lines) == 2:
        left_words = lines[0].replace("- ", "").split()
        right_words = lines[1].replace("- ", "").split()
        if left_words and left_words[-1].lower() in weak | {"i", "it", "who", "what", "this", "that"}:
            return True
        if right_words and right_words[0].lower() in {"and", "or", "but", "to", "of", "for", "with", "because", "that", "this"}:
            return True
    return False
