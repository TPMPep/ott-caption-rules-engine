...
def process_caption_job(
    ...
    print("[FORMATTER] Formatting cues")
...
def fit_sound_into_dialogue_gap(
    ...
    # Let applause/music live longer if there is room before dialogue begins
...
def format_cues(cues: List[Dict[str, Any]], protected_phrases: List[str], max_rounds: int = 3) -> List[Dict[str, Any]]:
    ...
    for idx, cue in enumerate(cues):
        ...
        if is_two_speaker:
            left = fit_single_line(runs[0]["text"], MAX_CHARS - 2, protected_phrases)
            right = fit_single_line(runs[1]["text"], MAX_CHARS - 2, protected_phrases)

            # if either speaker line still needs more than one line, split into separate events
            if needs_split(runs[0]["text"], MAX_CHARS - 2) or needs_split(runs[1]["text"], MAX_CHARS - 2):
                split_events = split_two_speaker_cue(cue)
                child_results = format_cues(split_events, protected_phrases, max_rounds=max_rounds)
        ...
        broken = safe_split(text, MAX_CHARS, protected_phrases)
        ...
        if count_function_word_endings(cue["lines"]) > 0 and max_rounds > 0:
            improved = safe_split(text, MAX_CHARS, protected_phrases, prefer_balance=False)
            if improved:
...
def needs_split(text: str, max_chars: int) -> bool:
...
def safe_split(
    ...
    """
    Split only on word boundaries. Never raw character slicing across words.
    """
    text = clean_dialogue_text(text)
    if not text:
        return [""]
    ...
    # If it cannot fit in 2 lines cleanly, do a safe single-line trim and let caller split cue
...
def fit_single_line(text: str, max_chars: int, protected_phrases: List[str]) -> str:
    ...
    # avoid ending on a function word when possible
    while len(out) > 1 and out[-1].lower() in FUNCTION_WORDS:
        out.pop()
        candidate = " ".join(out).strip()

    # avoid splitting protected phrase tail when possible
    for phrase in protected_phrases:
        phrase_words = phrase.split()
        candidate_words = candidate.split()
        if candidate_words and len(candidate_words) < len(phrase_words):
            tail = phrase_words[:len(candidate_words)]
            if candidate_words == tail:
                out = out[:-1]
                candidate = " ".join(out).strip()
                break
